...
project_dir=${test_path}/kvm_coco_snapshot
logfile=${test_path}/test.log
...
echo "kvm-coco-queue has been updated on $tag_date https://git.kernel.org/pub/scm/virt/kvm/kvm.git kvm-coco-queue" | /usr/local/bin/mutt -e 'set sendmail_wait=0' -e 'set use_from=yes' -e 'set realname="Chen Yu"' -s "[notify] kvm-coco-queue rebase detected." -n yu.c.chen@intel.com,julie.du@intel.com
